...
import {ReactComponent as Cross} from "./cross.svg"; //временно
import {ReactComponent as ChevronDown} from "./chevron-down.svg"; //временно
import {ReactComponent as ChevronUp} from "./chevron-up.svg"; //временно
...
//закрыть при клике на внешнее поле
  ...
  const handleDisplayClickToToggleDropdown: MouseEventHandler<HTMLDivElement> = () => {
...
const handleClearSelection = (event: React.MouseEvent) => {
  ...
  // Рендер инпута
  const renderSearchableInput = (inputClassName: string, hasSelectedOptions: boolean) => (
    <input
      type="text"
  ...
  const renderStaticInput = (inputClassName: string) => (
    <div className={inputClassName} onClick={handleDisplayClickToToggleDropdown}>
      {getItemsText()}
    </div>
  );
  
  // Обработчик клика для поискового инпута
  const handledropdownInputClick = (hasSelectedOptions: boolean) => 
    (e: React.MouseEvent) => {
      e.stopPropagation();
      if (!isOpen) {
        setIsOpen(true);
...
const renderDropdownIcon  = () => {
  ...
  const hasContentToClear = searchQuery.length > 0 || selectedOptions.length > 0;
  ...
  return (
    <div className={clsx(styles.dropdownContainer, className)} ref={dropdownRef}>
      ...
      >
        <div className={clsx(styles.dropdownHeader, isOpen && styles.withBorder)}>
          {renderInputField()}
          {renderDropdownIcon ()}
        </div>
